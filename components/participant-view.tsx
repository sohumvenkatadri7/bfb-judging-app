...
const MILESTONES = [
  {
    value: 25,
    label: "Ideation Complete",
    description: "Problem statement finalized & solution designed",
  },
  {
    value: 50,
    label: "MVP Built",
    description: "Core prototype functional and testable",
  },
  {
    value: 75,
    label: "Integration Done",
    description: "All features connected and working end-to-end",
  },
  {
    value: 100,
    label: "Submission Ready",
    description: "Polished, documented, and demo-ready",
  },
];
...
function TimeUnit({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center">
      <div className="flex h-20 w-20 items-center justify-center rounded-xl border border-border bg-card font-mono text-3xl font-bold text-navy shadow-sm md:h-24 md:w-24 md:text-4xl">
...
export function ParticipantView({ team, onUpdateTeam }: ParticipantViewProps) {
  ...
  const getAlertIcon = (type: string) => {
    switch (type) {
      ...
      case "success":
        return <CheckCircle2 className="size-4 shrink-0 text-emerald" />;
      default:
        return <Info className="size-4 shrink-0 text-navy" />;
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        ...
        <p className="max-w-sm text-center text-sm text-muted-foreground">
          Your team could not be located. Please sign out and try again.
        </p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-8 pb-20 md:pb-8">
      ...
      <section className="animate-float-up">
        <div className="flex items-center gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="flex size-12 items-center justify-center rounded-xl bg-navy/10 font-mono text-lg font-bold text-navy">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <span className="font-mono text-xs text-muted-foreground">
                team{team.id}@bfb.com
              </span>
            </div>
          </div>
          {team.isTop20 && (
            <span className="rounded-full bg-emerald/10 px-3 py-1 text-xs font-semibold text-emerald-dark">
              Top 20
            </span>
          )}
        </div>
      ...
      <section className="animate-float-up" style={{ animationDelay: "0.1s" }}>
        <div className="mb-4 flex items-center gap-2">
          <Megaphone className="size-5 text-navy" />
        ...
        <div className="flex flex-col gap-2">
          {adminAlerts.map((alert) => (
            <button
              ...
              className="flex w-full items-start gap-3 rounded-xl border border-border bg-card p-4 text-left shadow-sm transition-all hover:border-slate-300"
            >
          ...
          {MILESTONES.map((ms, idx) => {
            ...
            return (
              <button
                key={ms.value}
                type="button"
                onClick={() => handleMilestoneClick(ms.value)}
                disabled={!isNext}
                className={`group relative flex flex-col items-center gap-4 rounded-2xl border p-6 text-center transition-all duration-300 md:p-8
                  ${
                    isCompleted
                      ? "border-emerald/30 bg-emerald-light/40"
                      : isNext
                        ? "cursor-pointer border-navy/20 bg-card shadow-sm hover:border-navy/40 hover:shadow-md"
              ...
              >
                  ...
                  {isCompleted ? (
                    ...
                    <span
                      className={`font-mono text-lg font-bold ${
                        isNext ? "text-navy" : "text-muted-foreground"
                      }`}
                    >
                      {idx + 1}
                    </span>
                ...
                {isNext && (
                  <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 rounded-full bg-navy px-3 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary-foreground shadow-sm">
                    Tap to complete
                  </span>
                )}
              </button>
